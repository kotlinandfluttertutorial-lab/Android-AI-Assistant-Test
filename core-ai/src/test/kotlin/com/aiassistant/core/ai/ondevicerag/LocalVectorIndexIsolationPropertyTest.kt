...
class LocalVectorIndexIsolationPropertyTest :
    DescribeSpec({
...
private class InMemoryOnDeviceChunkDaoV2 : com.aiassistant.core.database.dao.OnDeviceChunkDao {
    ...
    override suspend fun getAllChunks(userId: String): List<OnDeviceChunkEntity> = store.filter { it.userId == userId }
    ...
    override suspend fun countChunks(userId: String): Int = store.count { it.userId == userId }
