 ...
 * Pattern Used       : JUnit 4 + MockK + Coroutines Test
 *
...
class OnDeviceInferenceClientTest {
    ...
    private lateinit var client: OnDeviceInferenceClient

    @Before
    fun setUp() {
        ramMonitor = mockk(relaxed = true)
    ...
    private fun createClientWithModel(file: File? = null): OnDeviceInferenceClient =
        OnDeviceInferenceClient(ramMonitor = ramMonitor, modelFile = file)

    // ─── Model not available ─────────────────────────────────────────────────

    @Test
    fun `connect emits StreamEvent Error when model file is null`() = runTest {
    ...
    @Test
    fun `connect emits StreamEvent Error when model file does not exist`() = runTest {
        val nonExistent = File(tmpFolder.root, "ghost.gguf")
    ...
    @Test
    fun `connect emits Token events and ends with Done after sending message`() = runTest {
        val modelFile = tmpFolder.newFile("model.gguf").also { it.writeText("fake model") }
    ...
    @Test
    fun `Done event contains non-negative token counts`() = runTest {
        val modelFile = tmpFolder.newFile("model2.gguf").also { it.writeText("fake model") }
        val client = createClientWithModel(modelFile)
    ...
    @Test
    fun `connect emits Error when RAM is below threshold before inference`() = runTest {
        val modelFile = tmpFolder.newFile("model3.gguf").also { it.writeText("fake model") }
        // RAM below 512 MB
    ...
    @Test
    fun `sendMessage after disconnect is a no-op`() = runTest {
        val modelFile = tmpFolder.newFile("model4.gguf").also { it.writeText("fake model") }
        val client = createClientWithModel(modelFile)
    ...
    @Test
    fun `ON_DEVICE_PROVIDER_ID constant is non-blank`() {
