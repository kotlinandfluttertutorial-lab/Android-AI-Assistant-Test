...
class QueryRoutingLogRepositoryImpl @Inject constructor(
    ...
    override suspend fun logDecision(userId: String, decision: OnDeviceRoutingDecision): ApiResult<Unit> =
    ...
    @Suppress("TooGenericExceptionCaught")
    override suspend fun getRecentDecisions(userId: String, limit: Int): ApiResult<List<OnDeviceRoutingDecision>> =
        withContext(dispatchers.io) {
            try {
    ...
    @Suppress("TooGenericExceptionCaught")
    override suspend fun deleteOlderThan(cutoffMs: Long): ApiResult<Unit> = withContext(dispatchers.io) {
        try {
            ...
            ApiResult.Error(
                DomainError.ServerError(
                    "Failed to prune routing logs: ${e.message}",
                    HTTP_SERVER_ERROR
                )
            )
        }
    ...
    private fun QueryRoutingLogEntity.toDomain() = OnDeviceRoutingDecision(
        path = if (selectedPath == "ON_DEVICE") {
            OnDeviceInferencePath.ON_DEVICE
        } else {
            OnDeviceInferencePath.CLOUD
        },
