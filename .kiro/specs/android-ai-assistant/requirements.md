...
- **Gemma**: The on-device generative language model (Google Gemma 2B or 7B in quantized INT4/INT8 form) used exclusively as the generation and reasoning component in the On_Device_RAG pipeline. Gemma is NOT responsible for retrieval; it receives pre-assembled context from the retriever and produces a grounded answer.
- **On_Device_RAG**: The complete on-device Retrieval-Augmented Generation stack consisting of the On_Device_Embedding_Model, Local_Vector_Index, Chunker, and Gemma, all executing locally on the Android device without requiring a network connection.
- **Query_Router**: The decision component that evaluates device capability, connectivity, and model availability at query time and routes each request either to the On_Device_RAG path (Local RAG + Gemma) or to the cloud API path (Backend + remote LLM_Provider).
- **Local_Vector_Index**: The on-device vector index (backed by a SQLite-based or binary flat-file store) that persists Chunk Embeddings generated by the On_Device_Embedding_Model and supports approximate nearest-neighbour similarity search entirely in-process.
- **On_Device_Embedding_Model**: The mobile-optimised text embedding model (e.g., MiniLM-L6 or equivalent ≤50 MB) that runs on-device to generate fixed-dimension vector representations of both document Chunks and user queries for the Local_Vector_Index.
- **Chunker**: The on-device component that segments extracted document text into overlapping fixed-size Chunks suitable for embedding and storage in the Local_Vector_Index, mirroring the server-side RAG chunking logic but executing entirely on-device.
- **On_Device_Inference_Engine**: The Android runtime layer that loads, manages, and executes Gemma using the MediaPipe LLM Inference API or equivalent framework, abstracting hardware-specific acceleration (CPU, GPU, NPU) and exposing a streaming token generation interface to the domain layer.
...
### Requirement 32: On-Device Gemma Inference Engine

**User Story:** As a User, I want the app to load and run the Gemma language model directly on my device, so that I can generate AI responses locally with no network dependency and full data privacy.

#### Acceptance Criteria

1. WHERE the device reports a Neural Processing Unit (NPU) or GPU with at least 4 GB of available dedicated memory, THE On_Device_Inference_Engine SHALL load a quantized Gemma model (INT4 or INT8) from local storage using the MediaPipe LLM Inference API or an equivalent on-device inference framework, and SHALL confirm successful load by emitting a `model_ready` status event within 30 seconds of initialization.
2. WHEN Gemma is loaded and a prompt is submitted, THE On_Device_Inference_Engine SHALL begin streaming generated tokens to the presentation layer within 2,000 ms of prompt submission on a device meeting the NPU/GPU threshold in criterion 1.
3. WHEN the On_Device_Inference_Engine streams a response, THE AI_Assistant SHALL display each token incrementally in the chat view as it is produced, using the same Streaming_Response rendering path used for cloud LLM_Providers.
4. WHEN the On_Device_Inference_Engine completes a generation request, THE AI_Assistant SHALL record and display the following performance metrics for that request: time-to-first-token (ms), total generation time (ms), tokens per second, and the hardware accelerator used (CPU / GPU / NPU).
5. THE On_Device_Inference_Engine SHALL expose a performance benchmark mode accessible from the Settings screen that runs a fixed 200-token prompt ten times consecutively and reports mean and p95 values for time-to-first-token and tokens-per-second, broken down by CPU, GPU, and NPU where each is available.
6. IF the On_Device_Inference_Engine fails to load the Gemma model (corrupt files, insufficient memory, unsupported hardware), THEN THE AI_Assistant SHALL log the failure reason, display a structured error message to the User, and automatically route subsequent requests to the configured cloud LLM_Provider fallback.
7. WHEN the On_Device_Inference_Engine is active, THE AI_Assistant SHALL display a persistent "Running on device" indicator in the chat toolbar so the User is always aware that no data is transmitted externally.
8. THE On_Device_Inference_Engine SHALL support graceful cancellation: WHEN the User taps the stop-generation control, THE On_Device_Inference_Engine SHALL halt token emission within 500 ms and release the generation thread without terminating the loaded model.
9. WHEN the app process is moved to the background while Gemma is idle (not actively generating), THE On_Device_Inference_Engine SHALL release GPU/NPU memory within 5 seconds to allow the operating system to reclaim resources, and SHALL reload the model on the next foreground request within the time limit defined in criterion 1.
10. THE On_Device_Inference_Engine SHALL verify the integrity of the Gemma model files using a SHA-256 checksum against a bundled manifest before loading. IF the checksum does not match, THE On_Device_Inference_Engine SHALL treat the files as corrupt and trigger the download prompt defined in criterion 6.

---

### Requirement 33: On-Device Document Ingestion

**User Story:** As a User, I want to import documents directly on my device and have them processed locally, so that my document content never leaves my device and is available for on-device RAG queries even when offline.

#### Acceptance Criteria

1. THE AI_Assistant SHALL accept local document imports in PDF, TXT, and Markdown formats from the device file picker or share intent, with a maximum file size of 50 MB per document. IF a document exceeds the size limit or is in an unsupported format, THE AI_Assistant SHALL display a structured error identifying the file name and the reason for rejection before any processing begins.
2. WHEN a PDF document is submitted for on-device ingestion, THE AI_Assistant SHALL extract text using a local PDF parsing library (e.g., PdfRenderer or iText). IF the PDF is a scanned image-only document and on-device OCR is unavailable, THE AI_Assistant SHALL display a descriptive error indicating that scanned PDFs require the cloud ingestion path.
3. WHEN a TXT or Markdown document is submitted for on-device ingestion, THE AI_Assistant SHALL read the raw text content directly without invoking any network service.
4. WHEN text extraction is complete, THE Chunker SHALL split the extracted text into overlapping Chunks with configurable parameters (default: 512 tokens per chunk, 64-token overlap, minimum 64 tokens, maximum 2,048 tokens, maximum overlap 50% of chunk size) such that the union of all Chunks covers the full extracted text with no gaps.
5. THE AI_Assistant SHALL store each Chunk and its metadata (document name, page number or character offset range, chunk index, creation timestamp) in the local Room database under the `on_device_chunks` table, associated exclusively with the authenticated User's local identifier.
6. WHEN on-device ingestion completes successfully, THE AI_Assistant SHALL display the document in the on-device documents list with a status of `ready` and the total number of Chunks produced. WHEN ingestion fails at any stage (extraction, chunking, or embedding), THE AI_Assistant SHALL display the document with a status of `failed` and a description of the failure stage.
7. THE AI_Assistant SHALL allow the User to delete any on-device document. WHEN a document is deleted, THE AI_Assistant SHALL remove all associated Chunks from the Room database and all associated Embeddings from the Local_Vector_Index within 10 seconds.
8. THE AI_Assistant SHALL support a round-trip ingestion property: for any valid TXT or Markdown document, ingesting the document and then querying a verbatim phrase of at least 3 consecutive words present in the document SHALL return at least one Chunk containing that phrase from the Local_Vector_Index.
9. WHILE on-device document ingestion is in progress, THE AI_Assistant SHALL display an in-progress status indicator for that document in the on-device documents list and SHALL NOT block the User from interacting with other app features.
10. IF on-device storage available to the app falls below 100 MB during ingestion, THEN THE AI_Assistant SHALL pause ingestion, display a low-storage warning, and require the User to free space before resuming.

---

### Requirement 34: On-Device Embedding and Vector Index

**User Story:** As a User, I want document embeddings generated and searched entirely on my device, so that semantic retrieval works offline and my text data is never sent to an embedding API.

#### Acceptance Criteria

1. THE On_Device_Embedding_Model SHALL be a mobile-optimised text embedding model no larger than 50 MB on disk (e.g., MiniLM-L6-v2 quantized or equivalent), bundled with the application or downloaded on first use, capable of generating fixed-dimension Embeddings (minimum 384 dimensions) for text inputs up to 512 tokens.
2. WHEN the Chunker produces Chunks during on-device ingestion, THE On_Device_Embedding_Model SHALL generate an Embedding for each Chunk and THE AI_Assistant SHALL store the Embedding in the Local_Vector_Index keyed by the Chunk's unique identifier and scoped to the authenticated User.
3. THE Local_Vector_Index SHALL support cosine similarity search and SHALL return ranked results for a query Embedding within 500 ms for a corpus of up to 10,000 stored Chunk Embeddings on a mid-range Android device (Snapdragon 700 series or equivalent).
4. THE Local_Vector_Index SHALL persist all Embeddings to durable local storage (SQLite BLOB columns or a binary flat file) so that Embeddings survive app restarts and device reboots without requiring re-ingestion.
5. THE On_Device_Embedding_Model SHALL generate identical Embeddings for identical input text on the same device across separate app sessions, ensuring that similarity search results are reproducible (deterministic embedding property).
6. THE AI_Assistant SHALL verify the integrity of the On_Device_Embedding_Model files using a SHA-256 checksum against a bundled manifest at startup. IF the checksum does not match, THE AI_Assistant SHALL re-download the model before accepting any embedding requests.
7. WHEN the On_Device_Embedding_Model is loading for the first time (download or initial warm-up), THE AI_Assistant SHALL display a loading indicator on the on-device RAG feature entry point and SHALL NOT permit embedding generation requests until the model reports `ready`.
8. THE Local_Vector_Index SHALL enforce per-User isolation: a query scoped to User A SHALL NOT return Embeddings stored under User B's scope, even if both users are present in the local database.
9. THE AI_Assistant SHALL expose a diagnostic metric in the Settings screen showing the current Local_Vector_Index size in MB and total number of indexed Chunks, updated within 5 seconds of any change.
10. IF the Local_Vector_Index storage file becomes corrupt (detected via checksum mismatch or SQLite integrity check failure), THEN THE AI_Assistant SHALL display a recovery prompt offering to rebuild the index from the stored Chunks, and SHALL NOT serve retrieval results from a corrupt index.

---

### Requirement 35: On-Device RAG Query Pipeline

**User Story:** As a User, I want my questions answered using my locally stored documents, with Gemma generating the final response from retrieved context, so that I get grounded answers entirely on-device.

#### Acceptance Criteria

1. WHEN a User submits a query in the on-device RAG chat mode, THE On_Device_Embedding_Model SHALL generate an Embedding for the query text and THE Local_Vector_Index SHALL retrieve the top-K most semantically similar Chunks (default K=5, configurable by the User between 1 and 10 in Settings) from the authenticated User's indexed documents.
2. WHEN top-K Chunks are retrieved, THE AI_Assistant SHALL assemble a context window by concatenating the Chunk texts with their source document name and character offset range, within the token budget of the loaded Gemma model's context window (maximum 4,096 tokens by default unless the loaded model supports a larger context).
3. WHEN the context window is assembled, THE AI_Assistant SHALL construct a prompt that instructs Gemma to answer using only the provided context, clearly identifying that Gemma is the generation component and that it MUST NOT fabricate information absent from the retrieved Chunks.
4. WHEN Gemma generates a response in the on-device RAG pipeline, THE AI_Assistant SHALL include inline citations referencing the source document name and character offset range for each retrieved Chunk that contributed to the response, displayed in the same citation format as the cloud RAG pipeline defined in Requirement 4 criterion 7.
5. IF the top-K retrieval step returns zero Chunks with a cosine similarity above 0.40, THEN THE AI_Assistant SHALL display a "No relevant content found in local documents" message to the User and SHALL NOT invoke Gemma for that query.
6. WHEN the on-device RAG pipeline produces a complete response, THE AI_Assistant SHALL record the following query metrics in the local Room database: query text SHA-256 hash, number of Chunks retrieved, retrieval latency (ms), Gemma generation latency (ms), total pipeline latency (ms), and tokens generated.
7. THE on-device RAG query pipeline SHALL preserve the strict architectural separation that Gemma is ONLY the generation component: Gemma SHALL receive pre-assembled context text and a generation instruction, and SHALL NOT be invoked for embedding generation, retrieval scoring, or document parsing at any point in the pipeline.
8. WHEN a User views the response in on-device RAG mode, THE AI_Assistant SHALL provide a "Show sources" control that expands to display the full text of each retrieved Chunk along with its source document name, chunk index, and cosine similarity score (rounded to two decimal places).
9. IF Gemma fails to generate a response (model error, context overflow, generation timeout after 60 seconds), THEN THE AI_Assistant SHALL display a structured error message, log the failure with the pipeline stage and error code, and offer the User the option to retry the query via the cloud API path.
10. THE on-device RAG pipeline SHALL enforce a round-trip correctness property: for any on-device document, submitting a verbatim phrase of at least 5 words present in that document as a query SHALL return a Gemma response that references the source document and includes at least one of the retrieved Chunks containing the phrase.

---

### Requirement 36: Hybrid Query Router

**User Story:** As a User, I want my queries automatically routed to the best available inference path — on-device RAG or cloud API — based on what my device supports and whether I am online, so that I always get the best possible response without manual configuration.

#### Acceptance Criteria

1. THE Query_Router SHALL evaluate the following capability signals before each query: (a) whether on-device Gemma model files are present and their checksum is valid, (b) whether the On_Device_Embedding_Model is loaded and ready, (c) whether the Local_Vector_Index contains at least one Chunk for the authenticated User, and (d) whether the device has an active network connection to the Backend.
2. WHEN all four capability signals in criterion 1 are satisfied, THE Query_Router SHALL default to the on-device RAG path. THE User SHALL be able to override this default from the Settings screen to always prefer the cloud path or always prefer the on-device path.
3. WHEN the device has no network connectivity and on-device capability signals (a), (b), and (c) from criterion 1 are satisfied, THE Query_Router SHALL route exclusively to the on-device path and SHALL NOT attempt a cloud API call.
4. WHEN the device has no network connectivity and on-device capability signals are not fully satisfied, THE Query_Router SHALL display a structured offline message to the User explaining which capability is unavailable and SHALL NOT silently queue the query for later delivery.
5. WHEN the Query_Router routes a query to the cloud API path, THE AI_Assistant SHALL display a "Using cloud AI" indicator and SHALL use the active LLM_Provider configured in Requirement 3. WHEN the Query_Router routes to the on-device path, THE AI_Assistant SHALL display a "Running on device" indicator as defined in Requirement 32 criterion 7.
6. WHEN a cloud API call fails with a connection error or a 5xx HTTP response, THE Query_Router SHALL automatically fall back to the on-device path if on-device capability signals (a), (b), and (c) from criterion 1 are satisfied, notify the User that the cloud call failed and on-device inference is being used, and record the fallback event in the local analytics log.
7. WHEN a fallback from cloud to on-device occurs per criterion 6, THE Query_Router SHALL NOT repeat the cloud attempt for the same query. IF the User explicitly retries, THE Query_Router SHALL re-evaluate all capability signals before selecting a path.
8. THE Query_Router SHALL expose routing decision metadata to the presentation layer so that the UI can display the active path (on-device or cloud), the reason for the selected path, and the fallback history for the current session.
9. IF the User has configured an explicit path preference (always on-device or always cloud) and the preferred path becomes unavailable, THE Query_Router SHALL display a structured error identifying the unavailable path and SHALL NOT silently switch to the non-preferred path without informing the User.
10. THE Query_Router SHALL record each routing decision in the local Room analytics table with: timestamp, selected path, capability signals evaluated (as a bitmask), User override setting, and whether a fallback occurred. These records SHALL be retained for 30 days.

---

### Requirement 37: On-Device Performance and Resource Management

**User Story:** As a User, I want on-device AI to run efficiently without draining my battery or making my device unresponsive, so that I can use the feature throughout the day without noticing a performance penalty.

#### Acceptance Criteria

1. WHILE an on-device Gemma generation request is executing, THE On_Device_Inference_Engine SHALL monitor available device RAM every 2 seconds. IF available RAM falls below 512 MB during inference, THE On_Device_Inference_Engine SHALL cancel the current generation, display an "Insufficient memory — switching to cloud" message, and automatically retry the same prompt against the configured cloud LLM_Provider fallback.
2. THE On_Device_Inference_Engine SHALL honor Android's power-saving mode: WHILE the device is in Battery Saver mode, THE On_Device_Inference_Engine SHALL restrict inference to CPU-only execution and SHALL display a "Battery saver active — on-device AI uses CPU only" notice to the User. GPU and NPU acceleration SHALL resume automatically when Battery Saver mode is deactivated.
3. WHEN Gemma model files or On_Device_Embedding_Model files need to be downloaded or updated, THE AI_Assistant SHALL initiate the download only when the device is connected to an unmetered network (Wi-Fi), unless the User explicitly grants permission to use mobile data. The download SHALL use Android WorkManager with a `NetworkType.UNMETERED` constraint.
4. WHEN a model download is in progress, THE AI_Assistant SHALL display download progress as a percentage and estimated remaining time. IF the download is interrupted, THE AI_Assistant SHALL resume from the last verified byte on the next eligible network connection without restarting from zero.
5. WHEN a new version of the Gemma model or On_Device_Embedding_Model is available, THE AI_Assistant SHALL notify the User with an in-app prompt (not a push notification) and SHALL continue using the existing model version until the User approves the update download.
6. THE On_Device_Inference_Engine SHALL complete a response for a 200-token input prompt within 30 seconds on a device meeting the NPU/GPU threshold defined in Requirement 32 criterion 1. IF generation has not completed within 30 seconds, THE On_Device_Inference_Engine SHALL cancel the request, display a timeout message, and offer the cloud path as an alternative.
7. THE AI_Assistant SHALL record device thermal state (nominal, warm, hot, critical) at the start and end of each on-device inference request. WHILE the device thermal state is `critical`, THE On_Device_Inference_Engine SHALL pause inference, display a "Device too warm — pausing on-device AI" message, and check thermal state every 30 seconds until it returns to `hot` or below before resuming.
8. THE AI_Assistant SHALL provide a Resource Usage screen in Settings displaying, for the current app session: total on-device inference requests, total tokens generated on-device, cumulative on-device inference time (seconds), approximate CPU energy consumed (mWh, if available from the Android BatteryManager API), and current Local_Vector_Index disk usage.
9. WHEN the app is uninstalled, ALL on-device model files, Local_Vector_Index files, and on-device document data stored by the AI_Assistant SHALL be removed by the Android package manager through standard `getFilesDir()` or `getExternalFilesDir()` storage paths, leaving no residual data on the device.
10. THE On_Device_Inference_Engine SHALL support model file management: THE AI_Assistant SHALL provide a "Manage on-device models" screen in Settings showing each downloaded model's name, version, disk size, and last-used date, and SHALL allow the User to delete any model to reclaim storage. IF a deleted model is needed for inference, THE AI_Assistant SHALL trigger the download flow defined in criterion 3.

---

### Requirement 38: On-Device RAG Portfolio Documentation

**User Story:** As a developer showcasing this project, I want comprehensive portfolio-quality documentation of the on-device RAG architecture, benchmarks, and privacy guarantees, so that reviewers and collaborators can understand the system's design, performance, and differentiators at a glance.

#### Acceptance Criteria

1. THE project SHALL include a dedicated Markdown document at `docs/on-device-rag.md` that covers all six implementation phases of the On_Device_RAG system: (1) Gemma loading and performance measurement, (2) document ingestion and chunking, (3) on-device embedding and similarity search, (4) full RAG query pipeline, (5) offline mode and resource management, and (6) portfolio artifacts. Each phase section SHALL contain a purpose statement, a component list, and at least one design decision with rationale.
2. THE `docs/on-device-rag.md` document SHALL include a Mermaid architecture diagram showing the full six-layer stack (Jetpack Compose UI → Presentation → Domain → AI Engine → Local Data → Storage) and the two query paths (on-device: Query_Router → On_Device_Embedding_Model → Local_Vector_Index → Gemma; cloud: Query_Router → Backend → LLM_Provider), with all named components matching the Glossary in this document.
3. THE project README SHALL include a dedicated "On-Device RAG" section referencing `docs/on-device-rag.md` and listing: the on-device model(s) used with version numbers, minimum hardware requirements, supported document formats, and a one-paragraph plain-language explanation of how Gemma is used exclusively as a generation component (not a retrieval component).
4. THE `docs/on-device-rag.md` document SHALL include a benchmark results table with the following columns: Device Model, Chipset, Accelerator (CPU/GPU/NPU), Gemma Model Variant, Time-to-First-Token (p50 ms), Tokens per Second (p50), and RAM Usage Peak (MB). The table SHALL contain at minimum two rows covering at least two distinct hardware configurations.
5. THE project SHALL include an offline demo section in `docs/on-device-rag.md` that provides step-by-step instructions for demonstrating the full on-device RAG pipeline without any network connection, including: how to put the device in airplane mode, which sample documents to load, which sample queries to run, and the expected outputs including citations.
6. THE `docs/on-device-rag.md` document SHALL include a Privacy and Security section that explains: (a) which data never leaves the device during on-device RAG operation, (b) where model files are stored and how their integrity is verified, (c) how User documents are isolated in local storage, and (d) how the Query_Router ensures that on-device-only queries are never inadvertently forwarded to the cloud.
7. EVERY source file added for the On_Device_RAG feature (On_Device_Inference_Engine, Chunker, Local_Vector_Index, Query_Router, and related Use Cases and ViewModels) SHALL include the Educational_Header block defined in Requirement 22 criterion 4, with the `architectural placement` field referencing the specific layer in the six-layer stack described in criterion 1.
8. THE project SHALL include a `benchmarks/` directory containing at minimum one reproducible benchmark script (shell script or Gradle task) that installs the app on a connected Android device, runs the performance benchmark mode defined in Requirement 32 criterion 5, captures the output, and saves it to a timestamped JSON file in `benchmarks/results/`. The script SHALL be documented in `docs/on-device-rag.md`.
