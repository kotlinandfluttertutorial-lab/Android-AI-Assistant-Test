...
graph TD
    ...
    FEATURE_ON_DEVICE_RAG --> CORE_AI
    FEATURE_ON_DEVICE_RAG --> CORE_DATABASE
...
### On-Device RAG Architecture Overview

The On-Device RAG system is a six-layer stack that runs entirely on the Android device. It is composed of two independent pipelines: a **Document Ingestion Pipeline** that preprocesses and indexes uploaded files, and a **Query Pipeline** that retrieves relevant context and generates grounded answers using the on-device Gemma model. A **Query Router** decides at request time whether to use the on-device path or fall back to the cloud API.

#### Six-Layer On-Device Stack

```mermaid
graph TB
    subgraph L1["Layer 1 — Jetpack Compose UI"]
        UI_CHAT["OnDeviceRagChatScreen"]
        UI_DOCS["OnDeviceDocumentsScreen"]
        UI_SETTINGS["OnDeviceSettingsScreen / BenchmarkScreen"]
    end

    subgraph L2["Layer 2 — Presentation"]
        VM_RAG["OnDeviceRagViewModel\n(UiState + ChatEvents)"]
        VM_DOC["OnDeviceDocumentViewModel"]
    end

    subgraph L3["Layer 3 — Domain"]
        UC_INGEST["OnDeviceIngestDocumentUseCase"]
        UC_QUERY["OnDeviceQueryUseCase"]
        UC_ROUTE["RouteQueryUseCase"]
        UC_BENCH["BenchmarkOnDeviceUseCase"]
        UC_MODELS["ManageOnDeviceModelsUseCase"]
    end

    subgraph L4["Layer 4 — AI Engine (core-ai)"]
        ROUTER["QueryRouter"]
        EMBEDDER["OnDeviceEmbeddingModel"]
        CHUNKER["Chunker"]
    ...
    subgraph L5["Layer 5 — Local Data (core-database)"]
        ROOM_CHUNKS["Room: on_device_chunks"]
        ROOM_DOCS["Room: on_device_documents"]
        ROOM_LOG["Room: query_routing_log"]
    end

    subgraph L6["Layer 6 — Storage"]
        FS_MODELS["FileSystem: Gemma model files\n(getFilesDir / MediaPipe)"]
        FS_EMBED["FileSystem: EmbeddingModel files"]
        FS_INDEX["SQLite: LocalVectorIndex BLOB store"]
    end
...
#### Hybrid Query Routing Decision

```mermaid
flowchart TD
    QUERY["User Query"] --> ROUTER["QueryRouter\n(evaluates 4 capability signals)"]

    ROUTER --> CHECK{"All 4 signals\nsatisfied?"}
    CHECK -- "Yes + no override\nor prefer_on_device" --> ONDEV["On-Device Path\nLocal RAG + Gemma"]
    CHECK -- "No, or prefer_cloud" --> CLOUD["Cloud Path\nBackend + LLM_Provider"]

    ONDEV --> ANSWER["Final Answer\n(with citations)"]
    CLOUD --> ANSWER

    subgraph Signals["Capability Signals (bitmask)"]
        S0["bit 0 — Gemma model files present & checksum valid"]
        S1["bit 1 — OnDeviceEmbeddingModel loaded & ready"]
        S2["bit 2 — LocalVectorIndex has ≥1 chunk for user"]
        S3["bit 3 — Active network connection to Backend"]
    end

    ROUTER -.->|evaluates| Signals

    CLOUD -- "connection error / 5xx" --> FALLBACK{"On-device\ncapable?"}
    FALLBACK -- "Yes" --> ONDEV
    FALLBACK -- "No" --> ERROR["Structured error\n(no path available)"]
```
...
```mermaid
flowchart LR
    SRC["Document Source\n(File Picker / Share Intent)\nPDF · TXT · MD\n≤50 MB"] --> PARSER["DocumentParser\n(PdfRenderer for PDF\nRaw read for TXT/MD)"]
    PARSER --> EXTRACTOR["TextExtractor\n(native text or OCR fallback)"]
    EXTRACTOR --> CHUNKER["Chunker\n(512 tokens, 64 overlap\nmin 64, max 2048)"]
    CHUNKER --> EMBEDDER["OnDeviceEmbeddingModel\n(MiniLM-L6 ≤50 MB\ngenerates FloatArray per chunk)"]
    EMBEDDER --> INDEX["LocalVectorIndex\n(SQLite BLOB store\ncosine similarity)"]
    EMBEDDER --> ROOM["Room DB\non_device_chunks\n+ on_device_documents"]
    INDEX --> READY["status: ready\n(chunk count displayed)"]

    FAIL["Extraction / Chunking /\nEmbedding Error"] --> FAILSTATUS["status: failed\n(failureStage recorded)"]
```

**Ingestion status states:** `pending` → `processing` → `ready` (or `failed`). The `failureStage` field identifies which step failed (`extraction`, `chunking`, or `embedding`).

#### On-Device Query Pipeline

```mermaid
flowchart LR
    QUESTION["User Question"] --> QEMBED["OnDeviceEmbeddingModel\n(embed query → FloatArray)"]
    QEMBED --> SEARCH["LocalVectorIndex.search()\n(cosine similarity, top-K=5\nscoped to userId)"]
    SEARCH --> THRESHOLD{"cosine sim\n> 0.40?"}
    THRESHOLD -- "No results" --> NOMATCH["'No relevant content\nfound in local documents'"]
    THRESHOLD -- "Yes" --> CTXBUILD["ContextBuilder\n(assemble chunk texts +\nsource name + char offsets\nwithin Gemma context budget)"]
    CTXBUILD --> GEMMA["OnDeviceInferenceEngine\n(Gemma — generation only\nreceives: context + instruction)"]
    GEMMA --> RESPONSE["Grounded Answer\n(inline citations: doc name +\nchar offset range)"]
    RESPONSE --> METRICS["Record to Room:\nquery hash · chunks retrieved\nretrieval latency · gen latency\ntotal latency · tokens generated"]
```

**Critical architectural constraint — Gemma is generation-only.** The Gemma model object is never invoked for embedding generation, vector search scoring, or document parsing. These operations are exclusively handled by `OnDeviceEmbeddingModel` and `LocalVectorIndex` before Gemma receives any input.
...
| `feature-on-device-rag` | OnDeviceRagChatScreen, OnDeviceDocumentsScreen, OnDeviceDocumentChatScreen, BenchmarkScreen, ManageModelsScreen | `OnDeviceRagViewModel`, `OnDeviceDocumentViewModel` |
...
### On-Device RAG — `core-ai` Components

All five on-device RAG engine components live in the `core-ai` module. They have zero Android UI framework dependencies and are injectable via Hilt.

#### `OnDeviceInferenceEngine`

Wraps the MediaPipe LLM Inference API (or equivalent on-device inference framework) and exposes a coroutine-friendly streaming interface. Hardware accelerator detection and RAM monitoring are handled internally.

```kotlin
// core-ai: OnDeviceInferenceEngine interface
interface OnDeviceInferenceEngine {
    /**
     * Loads the Gemma model from [modelPath], verifying SHA-256 checksum against [expectedChecksum].
     * Emits [ModelLoadEvent.Ready] on success or [ModelLoadEvent.Failed] on error.
     ...
     * The caller provides pre-assembled context — Gemma receives only text.
     * Cancellation of the Flow triggers graceful generation halt within 500 ms.
     */
    fun generateStream(prompt: String): Flow<OnDeviceStreamEvent>

    /** Cancels any in-progress generation within 500 ms. */
    suspend fun cancelGeneration()

    /**
     * Runs a 200-token fixed prompt 10 times consecutively.
     * Returns mean and p95 TTFT and tokens/sec broken down by accelerator.
     */
    suspend fun benchmarkMode(): BenchmarkResult

    /** Returns the hardware accelerator in use: CPU, GPU, or NPU. */
    fun activeAccelerator(): HardwareAccelerator

    /**
     * Releases GPU/NPU memory when the model is idle.
     * Called by the lifecycle observer when the app goes to background.
     * Model can be reloaded on next foreground request.
     */
...
sealed class ModelLoadEvent {
    ...
    data class Failed(val reason: String, val cause: Throwable?) : ModelLoadEvent()
}
...
**RAM monitoring:** The `OnDeviceInferenceEngine` implementation polls `ActivityManager.MemoryInfo` every 2 seconds during generation. If available RAM drops below 512 MB, it emits `OnDeviceStreamEvent.Error(stage = "ram_exceeded")` and cancels generation, allowing the caller to route to the cloud fallback.

**Thermal management:** The implementation checks `PowerManager.thermalStatus` at generation start. If status is `THERMAL_STATUS_CRITICAL`, it defers and emits `Error(stage = "thermal_critical")`. It re-checks every 30 seconds until the thermal state returns to `THERMAL_STATUS_HOT` or below.

#### `OnDeviceEmbeddingModel`

Wraps the on-device MiniLM-L6-v2 (or equivalent ≤50 MB quantized) embedding model. Generates fixed-dimension `FloatArray` embeddings deterministically — identical input always produces identical output on the same device.

```kotlin
// core-ai: OnDeviceEmbeddingModel interface
interface OnDeviceEmbeddingModel {
    /**
     * Verifies model file checksum, then warms up the model.
     * Emits [ModelLoadEvent.Ready] or [ModelLoadEvent.Failed].
     */
    suspend fun initialize(modelPath: String, expectedChecksum: String): ModelLoadEvent

    /**
     * Generates a fixed-dimension FloatArray embedding for [text].
     * Deterministic: identical input produces identical output on the same device.
     * Text is truncated to 512 tokens if longer.
     */
    suspend fun generateEmbedding(text: String): FloatArray

    /** Dimension of generated embeddings (minimum 384). */
    val embeddingDimension: Int

    /** True when the model is loaded and ready to serve embeddings. */
    val isReady: Boolean
}
```

**Checksum verification:** At `initialize()`, the implementation reads the model file bytes, computes SHA-256, and compares against `expectedChecksum`. A mismatch sets `isReady = false` and triggers a re-download prompt via `ManageOnDeviceModelsUseCase`.

#### `LocalVectorIndex`

SQLite-backed cosine similarity index. Embeddings are stored as serialized `ByteArray` (little-endian IEEE 754 float32) in the `on_device_chunks` Room entity. Search is performed in-process using Kotlin-optimized dot-product computation after L2-normalizing stored and query vectors. Results are scoped to a single `userId` — cross-user queries are architecturally impossible.

```kotlin
// core-ai: LocalVectorIndex interface
interface LocalVectorIndex {
    /**
     * Stores [chunk] with its [embedding] under [userId] scope.
     * Overwrites an existing entry with the same [chunk.id].
     */
    suspend fun addChunk(userId: String, chunk: TextChunk, embedding: FloatArray)

    /**
     * Returns the top-[k] most semantically similar chunks for [queryEmbedding]
     * scoped exclusively to [userId]. Results below [minSimilarity] are excluded.
    ...
    /** Removes all chunks and embeddings for [documentId] under [userId]. */
    suspend fun deleteByDocument(userId: String, documentId: String)

    /** Returns current index stats for the Settings diagnostic screen. */
    suspend fun getStats(userId: String): VectorIndexStats
}
...
**User isolation guarantee:** The `search()` implementation includes a `WHERE userId = ?` clause at the SQL layer. No in-memory filter is relied upon; isolation is enforced at the data access level.

#### `Chunker`

Segments extracted document text into overlapping fixed-size chunks. The `tokenCount()` function uses a simple whitespace-based approximation (4 chars ≈ 1 token) for on-device use, consistent with the server-side chunker.

```kotlin
// core-ai: Chunker
class Chunker(
    private val chunkSizeTokens: Int = 512,
    private val overlapTokens: Int = 64,
    private val minChunkSizeTokens: Int = 64,
    private val maxChunkSizeTokens: Int = 2048
) {
    init {
        require(chunkSizeTokens in minChunkSizeTokens..maxChunkSizeTokens)
        require(overlapTokens <= chunkSizeTokens / 2) { "overlap must be ≤ 50% of chunk size" }
    }

    /**
     * Splits [text] into overlapping [TextChunk] segments.
     * The union of all chunk content covers the full [text] with no gaps.
     */
    fun chunk(
        text: String,
        documentId: String,
        documentName: String,
        pageOffsets: List<PageOffset> = emptyList()  // for page-number attribution
    ): List<TextChunk>
}

data class PageOffset(val pageNumber: Int, val startCharOffset: Long, val endCharOffset: Long)
```

#### `QueryRouter`

Evaluates four capability signals and returns a `RoutingDecision`. The decision is a pure function of the signal bitmask and the user's override setting — no side effects.

```kotlin
// core-ai: QueryRouter
interface QueryRouter {
    /**
     * Evaluates all four capability signals and returns a [RoutingDecision].
     * This is a pure function: identical signals + preference → identical decision.
     */
    suspend fun evaluate(userId: String, userPreference: PathPreference?): RoutingDecision
}
...
data class RoutingDecision(
    ...
    val capabilityBitmask: Int,  // bit 0=model, 1=embedding, 2=index, 3=network
    val reason: String,
...
**Bitmask semantics:**
| Bit | Signal | Condition for 1 |
|-----|--------|-----------------|
| 0 | Gemma model files | Files present and SHA-256 checksum valid |
| 1 | Embedding model | `OnDeviceEmbeddingModel.isReady == true` |
| 2 | Local vector index | `LocalVectorIndex` has ≥1 chunk for `userId` |
| 3 | Network | Active connection to Backend reachable |

**Default routing rule:** If bitmask bits 0–3 are all set AND `userPreference` is null or `PREFER_ON_DEVICE`, route `ON_DEVICE`. In all other cases (any signal unset, or `userPreference == PREFER_CLOUD`), route `CLOUD`. If network is unavailable (bit 3 = 0) and on-device signals (bits 0–2) are satisfied, route `ON_DEVICE` exclusively — never queue for later.

### On-Device RAG — Use Cases (Domain Module)

Five use cases in the `domain` module orchestrate the on-device RAG lifecycle. Each has a single `invoke` operator and zero side effects beyond repository/engine calls.

```kotlin
// domain: OnDeviceIngestDocumentUseCase
class OnDeviceIngestDocumentUseCase @Inject constructor(
    ...
    private val onDeviceDocumentRepository: OnDeviceDocumentRepository,
    private val dispatchers: DispatcherProvider
) {
    /**
     * Orchestrates: Parse → Extract → Chunk → Embed → Index → Persist.
     * Emits [IngestionProgress] events as a Flow for the ViewModel to observe.
     * On failure at any stage, records [failureStage] in the Room document record.
     */
    operator fun invoke(uri: Uri, userId: String): Flow<IngestionProgress>
}
...
sealed class IngestionProgress {
    ...
    data class Complete(val document: OnDeviceDocument, val totalChunks: Int) : IngestionProgress()
    data class Failed(val stage: String, val reason: String) : IngestionProgress()
}

// domain: OnDeviceQueryUseCase
class OnDeviceQueryUseCase @Inject constructor(
    private val embeddingModel: OnDeviceEmbeddingModel,
    private val vectorIndex: LocalVectorIndex,
    private val contextBuilder: ContextBuilder,
    private val inferenceEngine: OnDeviceInferenceEngine,
    private val queryMetricsRepository: QueryMetricsRepository,
    private val dispatchers: DispatcherProvider
) {
    /**
     * Orchestrates: Embed query → Retrieve top-K → Build context → Stream Gemma response.
     * Returns a Flow of [OnDeviceQueryEvent] for streaming display.
     * Records query metrics to Room on completion.
     */
    operator fun invoke(query: String, userId: String, topK: Int = 5): Flow<OnDeviceQueryEvent>
}

sealed class OnDeviceQueryEvent {
    data object Retrieving : OnDeviceQueryEvent()
    data class ChunksRetrieved(val count: Int) : OnDeviceQueryEvent()
    data object Generating : OnDeviceQueryEvent()
    ...
    data class Error(val stage: String, val message: String) : OnDeviceQueryEvent()
}
...
class RouteQueryUseCase @Inject constructor(
    ...
    /**
     * Evaluates routing signals and records the decision.
     * Returns the [RoutingDecision] for the ViewModel to act upon.
     */
    suspend operator fun invoke(userId: String, preference: PathPreference?): RoutingDecision
}

// domain: BenchmarkOnDeviceUseCase
class BenchmarkOnDeviceUseCase @Inject constructor(
    private val inferenceEngine: OnDeviceInferenceEngine
) {
    /** Runs the 10-iteration benchmark and returns [BenchmarkResult]. */
    suspend operator fun invoke(): BenchmarkResult
}

// domain: ManageOnDeviceModelsUseCase
class ManageOnDeviceModelsUseCase @Inject constructor(
    private val modelFileRepository: ModelFileRepository,
    private val inferenceEngine: OnDeviceInferenceEngine,
    private val embeddingModel: OnDeviceEmbeddingModel
) {
    /** Lists all downloaded on-device models with metadata. */
    suspend fun listModels(): List<OnDeviceModelInfo>

    /** Downloads [model] on an unmetered network using WorkManager. */
    suspend fun downloadModel(model: OnDeviceModelInfo, allowMetered: Boolean = false): Flow<DownloadProgress>

    /** Verifies model checksum and deletes files if corrupt. */
    suspend fun verifyModel(model: OnDeviceModelInfo): VerificationResult

    /** Permanently deletes model files from [getFilesDir()]. */
    suspend fun deleteModel(model: OnDeviceModelInfo)
}
```
...
### On-Device RAG — Room Entities

Three new Room entities support the on-device RAG system, added to the existing `core-database` module.

```kotlin
// core-database: On-Device RAG Room entities

@Entity(
    tableName = "on_device_chunks",
    indices = [
        Index(value = ["userId", "documentId"]),
        Index(value = ["userId"])
    ]
)
data class OnDeviceChunkEntity(
    ...
    val documentName: String,
    val chunkIndex: Int,
    val pageNumber: Int?,             // null for TXT/Markdown
    val startCharOffset: Long,
    val endCharOffset: Long,
    val content: String,
    val embeddingBlob: ByteArray,     // little-endian IEEE 754 float32, serialized FloatArray
    val createdAt: Long               // epoch ms
)

@Entity(tableName = "on_device_documents")
data class OnDeviceDocumentEntity(
    @PrimaryKey val id: String,
    val userId: String,
    val fileName: String,
    val mimeType: String,             // "application/pdf" | "text/plain" | "text/markdown"
    val sizeBytes: Long,
    val totalChunks: Int,
    val ingestionStatus: String,      // "pending" | "processing" | "ready" | "failed"
    val failureStage: String?,        // "extraction" | "chunking" | "embedding" | null
    val createdAt: Long               // epoch ms
)

@Entity(tableName = "query_routing_log")
data class QueryRoutingLogEntity(
    @PrimaryKey val id: String,
    val userId: String,
    val timestamp: Long,              // epoch ms
    val selectedPath: String,         // "on_device" | "cloud"
    val capabilityBitmask: Int,       // bit 0=model, bit 1=embedding, bit 2=index, bit 3=network
    val userOverride: String?,        // "prefer_on_device" | "prefer_cloud" | null
...
interface OnDeviceChunkDao {
    ...
    @Query("SELECT * FROM on_device_chunks WHERE userId = :userId AND documentId = :documentId ORDER BY chunkIndex ASC")
    fun getChunksForDocument(userId: String, documentId: String): Flow<List<OnDeviceChunkEntity>>

    @Query("SELECT * FROM on_device_chunks WHERE userId = :userId ORDER BY createdAt DESC")
    fun getAllChunks(userId: String): Flow<List<OnDeviceChunkEntity>>
    ...
    @Query("SELECT SUM(LENGTH(embeddingBlob)) FROM on_device_chunks WHERE userId = :userId")
    suspend fun totalEmbeddingBytes(userId: String): Long?
}

interface OnDeviceDocumentDao {
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(doc: OnDeviceDocumentEntity)
    ...
    @Query("UPDATE on_device_documents SET ingestionStatus = :status, failureStage = :failureStage, totalChunks = :totalChunks WHERE id = :id")
...
interface QueryRoutingLogDao {
    @Insert
    suspend fun insert(entry: QueryRoutingLogEntity)

    @Query("SELECT * FROM query_routing_log WHERE userId = :userId ORDER BY timestamp DESC LIMIT :limit")
    fun getRecentLogs(userId: String, limit: Int = 100): Flow<List<QueryRoutingLogEntity>>

    @Query("DELETE FROM query_routing_log WHERE timestamp < :cutoffMs")
    suspend fun deleteOlderThan(cutoffMs: Long)  // retention: 30 days
}
```

**Embedding serialization:** `embeddingBlob` stores the `FloatArray` as a `ByteArray` of little-endian IEEE 754 float32 values (`FloatArray.size * 4` bytes). A `TypeConverter` pair in `core-database` handles `FloatArray ↔ ByteArray` conversion. The cosine similarity computation in `LocalVectorIndex` deserializes the blob back to `FloatArray` at query time.
...
*For any* valid on-device document (TXT or Markdown) and any verbatim phrase of at least 5 consecutive words present in that document, ingesting the document via the on-device pipeline and querying with that phrase SHALL return a Gemma response that references the source document name and includes at least one retrieved chunk whose content contains the queried phrase.
...
*For any* text string input, the `OnDeviceEmbeddingModel` SHALL produce byte-for-byte identical `FloatArray` embeddings across two independent calls on the same device, regardless of whether those calls occur within the same app session or after an app restart, provided the model files have not changed.
...
*For any* two distinct users A and B, if chunks have been added to the `LocalVectorIndex` under user A's scope, then a `search()` call scoped to user B SHALL return zero results that originated from user A's documents, regardless of the query embedding or the `k` parameter.
...
*For any* combination of capability signal bitmask and user path preference, the `QueryRouter` SHALL return `InferencePath.ON_DEVICE` if and only if all four capability signals are set (bitmask == 0b1111) AND the user preference is null or `PREFER_ON_DEVICE`; it SHALL return `InferencePath.CLOUD` in all other cases. No other factor SHALL influence the routing decision.
...
*For any* on-device RAG query, when the full `OnDeviceQueryUseCase` pipeline executes with a mock `OnDeviceInferenceEngine` instrumented to record all method invocations, the only methods invoked on the Gemma engine object SHALL be `generateStream()` and `cancelGeneration()`. No embedding generation (`OnDeviceEmbeddingModel.generateEmbedding`), vector index search (`LocalVectorIndex.search`), or document parsing call SHALL be dispatched to the `OnDeviceInferenceEngine` instance at any point during the pipeline execution.
...
**On-Device RAG Property Tests (Kotest PropTest, minimum 100 iterations):**

```kotlin
// Property 37: On-Device RAG Round-Trip
@Test
// Feature: android-ai-assistant, Property 37: On-Device RAG Round-Trip
fun `on-device RAG round-trip - querying verbatim phrase returns chunk containing phrase`() =
    runTest {
        checkAll(Arb.string(minSize = 100, maxSize = 2000)) { documentText ->
            // pick a verbatim 5-word phrase from the document
            val words = documentText.split(" ").filter { it.length > 2 }
            assume(words.size >= 5)
            val phrase = words.take(5).joinToString(" ")

            val userId = UUID.randomUUID().toString()
            val doc = createTxtDocument(documentText, userId)
            onDeviceIngestDocumentUseCase(doc.uri, userId).last() // wait for Complete

            val events = onDeviceQueryUseCase(phrase, userId).toList()
            val doneEvent = events.filterIsInstance<OnDeviceQueryEvent.Done>().firstOrNull()

            // either NoRelevantContent or Done with a citation referencing the source
            val noContent = events.any { it is OnDeviceQueryEvent.NoRelevantContent }
            if (!noContent) {
                assertNotNull(doneEvent)
                assertTrue(doneEvent!!.citations.any { it.documentName == doc.fileName })
            }
        }
    }
...
// Property 38: Embedding Determinism
@Test
// Feature: android-ai-assistant, Property 38: Embedding Determinism
fun `embedding model produces identical output for identical input`() =
    runTest {
        checkAll(Arb.string(minSize = 1, maxSize = 512)) { text ->
            val embedding1 = embeddingModel.generateEmbedding(text)
            val embedding2 = embeddingModel.generateEmbedding(text)
            assertContentEquals(embedding1, embedding2)
        }
    }

// Property 39: Local Vector Index User Isolation
@Test
// Feature: android-ai-assistant, Property 39: Local Vector Index User Isolation
fun `vector index search scoped to user A never returns user B chunks`() =
    runTest {
        checkAll(
            Arb.list(Arb.string(minSize = 10, maxSize = 200), range = 1..10),
            Arb.floatArray(Arb.float(-1f, 1f), size = 384)
        ) { textsB, queryEmbedding ->
            val userA = UUID.randomUUID().toString()
            val userB = UUID.randomUUID().toString()

            // insert chunks under user B only
            textsB.forEachIndexed { i, text ->
                val chunk = TextChunk(id = "$userB-$i", documentId = "doc-b", documentName = "b.txt",
                    chunkIndex = i, pageNumber = null, startCharOffset = 0L, endCharOffset = text.length.toLong(), content = text)
                val embedding = embeddingModel.generateEmbedding(text)
                localVectorIndex.addChunk(userB, chunk, embedding)
            }

            // search under user A — must return nothing from user B
            val results = localVectorIndex.search(userA, queryEmbedding, k = 10, minSimilarity = 0f)
            assertTrue(results.none { it.chunk.id.startsWith(userB) })
        }
    }

// Property 40: Query Router Path Selection
@Test
// Feature: android-ai-assistant, Property 40: Query Router Path Selection
fun `query router selects on-device path iff all signals set and no cloud preference`() =
    runTest {
        checkAll(
            Arb.int(0, 15),  // 4-bit bitmask
            Arb.element(listOf(null, PathPreference.PREFER_ON_DEVICE, PathPreference.PREFER_CLOUD))
        ) { bitmask, preference ->
            val allSignalsSet = bitmask == 0b1111
            val prefersCloud = preference == PathPreference.PREFER_CLOUD

            val decision = fakeQueryRouter(bitmask, preference)

            val expectedPath = if (allSignalsSet && !prefersCloud) InferencePath.ON_DEVICE else InferencePath.CLOUD
            assertEquals(expectedPath, decision.path)
        }
    }

// Property 41: Gemma Generation-Only Isolation
@Test
// Feature: android-ai-assistant, Property 41: Gemma Generation-Only Isolation
fun `on-device RAG pipeline never calls embedding or search methods on Gemma engine`() =
    runTest {
        checkAll(
            Arb.string(minSize = 5, maxSize = 200),  // query
            Arb.string(minSize = 100, maxSize = 1000) // document content
        ) { query, documentContent ->
            val spyEngine = SpyOnDeviceInferenceEngine()  // records all method calls
            val userId = UUID.randomUUID().toString()

            // ingest a document so retrieval has something to find
            val doc = createTxtDocument(documentContent, userId)
            onDeviceIngestDocumentUseCase(doc.uri, userId).last()

            // run the full query pipeline with the spy engine
            OnDeviceQueryUseCase(embeddingModel, localVectorIndex, contextBuilder, spyEngine, metricsRepo, dispatchers)
                .invoke(query, userId).toList()

            // Gemma must only have received generate/cancel calls — never embed/search/parse
            assertTrue(spyEngine.recordedCalls.none { it in setOf("generateEmbedding", "search", "parse") })
            assertTrue(spyEngine.recordedCalls.all { it in setOf("generateStream", "cancelGeneration", "releaseMemory") || it.isEmpty() })
        }
    }
```
