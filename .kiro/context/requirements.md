...
This project must follow Specification-Driven Development (SDD).

Never generate the entire application in a single response.

Work only on one specification (spec) at a time.
...
Only begin implementation after spec.md has been approved.

If implementation is requested, follow tasks.md exactly.

Never skip unfinished tasks.

Do not move to the next specification until all acceptance criteria are satisfied.
