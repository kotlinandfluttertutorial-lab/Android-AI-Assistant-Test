...
This walkthrough demonstrates the full privacy guarantee: device in airplane mode throughout,
no network activity, responses grounded in ingested documents.

### Prerequisites

- Android device with NPU or GPU ≥ 4 GB dedicated memory (CPU fallback works but is slower)
- Gemma 2B INT4 (~1.5 GB) downloaded via ManageModelsScreen
- At least one document ingested and showing a `"Ready"` badge

---

### Step 1 — Download a Model

> `aiassistant://open/ondevicerag/models`

Open **Manage On-Device Models** from Settings. Each model card shows name, version, and
formatted file size. Tap the download icon next to **Gemma 2B INT4**.

While downloading, the card shows a `LinearProgressIndicator` with live progress:
`"42% · 630 MB / 1.5 GB"`. On completion it transitions to `"Verifying checksum…"` —
SHA-256 is computed over the full file before the model is considered usable.

If Battery Saver is on, a banner appears at the top:
*"Battery saver active — on-device AI uses CPU only."* — inference will use the CPU
...
Open **On-Device Documents** and tap the **+** FAB to pick a file (PDF, TXT, or Markdown,
up to 50 MB).

The ingestion banner steps through three phases:

| Phase | Banner text |
|---|---|
| Parsing | *"Parsing report.pdf…"* + indeterminate progress bar |
| Chunking | *"Splitting report.pdf into chunks…"* + indeterminate progress bar |
| Embedding | *"Generating embeddings 12/47…"* + determinate progress bar |

When done, the document card shows a green `"Ready"` badge with its chunk count: *"47 chunks"*.

**Edge cases:**
- Files over 50 MB → snackbar: *"[filename] is too large. Maximum file size is 50 MB."*
- Less than 100 MB free storage → warning banner blocks ingestion:
  *"Storage is almost full. Free up space to continue ingesting documents."*

For a quick demo, use a plain text file with this sample content:
...
Turn on airplane mode in device Settings. The rest of the demo runs entirely offline.
With bit 3 (`NETWORK_REACHABLE`) unset, `QueryRouterImpl.evaluate()` always returns
`ON_DEVICE` regardless of any user preference setting.

---

### Step 4 — Query the Document

Tap the `"Ready"` document card to open **On-Device RAG Chat**.

Type the sample query and tap **Ask**:

> *"What embedding model does the assistant use?"*

Watch the toolbar progress through three visible states:

1. **No badge** → `"Checking capabilities…"` spinner (*Routing* state — bitmask being evaluated)
2. **No badge** → `"Searching local documents…"` spinner (*Searching* state — cosine similarity computed)
3. **"Running on device"** badge (`primaryContainer`) → response streams token-by-token (*Streaming* state)

When generation completes, tap **"Show sources (1)"** to expand the citation:

- Source document name and chunk index
- Page number (if PDF)
- Cosine similarity score, e.g. *"Similarity: 0.91"*
- The retrieved excerpt text

Airplane mode remains on throughout — no network activity occurs.

---

### Step 5 — Trigger NoRelevantContent (Optional)

Ask a question with no semantic match in the indexed documents:

> *"What is the boiling point of water?"*

If all chunk similarity scores fall below `DEFAULT_MIN_SIMILARITY = 0.40f`, the pipeline
emits a `NoRelevantContent` event and the screen displays:

> *"No relevant content found in local documents."*

No generation is attempted. This shows the system only answers from the ingested corpus —
it does not fall back to parametric knowledge.

---

### Step 6 — Observe Fallback (Optional)

Turn airplane mode off. Set routing preference to `PREFER_CLOUD` in Settings. Submit a query.

If on-device inference is unavailable, the `AnimatedVisibility` fallback banner slides in:

> *"On-device inference unavailable — using cloud AI."*

The toolbar badge changes to **"Using cloud AI"** (`secondaryContainer`). From an `Error`
state, the **"Retry via cloud"** button appears when `canRetry == true`.

---

### Step 7 — Run the Benchmark

> `aiassistant://open/ondevicerag/benchmark`

Open **On-Device Benchmark** from Settings. Tap **Run Benchmark**. The screen shows
*"Running N / 10 iterations…"* while the use case completes all 10 iterations with a
200-token fixed prompt.
...
The same metrics are also written to `benchmarks/results/benchmark_<timestamp>.json` when
run via `benchmarks/on_device_rag_benchmark.sh`.
...
|---|---|---|
| File > 50 MB | Documents | Snackbar: *"[filename] is too large. Maximum file size is 50 MB."* |
| Low storage (< 100 MB) | Documents | Warning banner — ingestion blocked |
| Ingestion failure | Documents | Red badge: *"Failed (parse)"* / *"Failed (embed)"* etc. |
| No relevant content (similarity < 0.40) | Chat | *"No relevant content found in local documents."* |
| On-device pipeline error | Chat | Error message + "Retry via cloud" + "New query" buttons |
| Cloud fallback occurred | Chat | Banner: *"On-device inference unavailable — using cloud AI."* |
| RAM below 512 MB during inference | Chat | Inference cancelled → Error state |
| Thermal critical at inference start | Chat | Error state, 30s delay before retry |
| Model checksum mismatch | ManageModels | *"Download failed: Checksum mismatch."* → re-download |
| Battery Saver active | ManageModels | Banner: *"Battery saver active — on-device AI uses CPU only."* |
