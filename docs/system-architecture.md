...
All traffic between the Android client and backend passes over HTTPS/WSS through an Nginx reverse proxy with TLS termination.
              ...
              └─► WebSocket Router (JWT validation)
                    └─► AI Orchestrator
                          ...
                          └─► MCP Broker (tool calls if needed)
                                └─► {"type":"tool_call","toolName":"...","toolInput":{...}}  ──► App
```

## Background Job Flow — RAG Ingestion

```
Android App  ──upload──►  /documents  ──►  MinIO (raw file)
...
| `chromadb` | `chromadb/chroma` | 8000 | Vector store |
| `minio` | `minio/minio` | 9000 | Document object storage |
